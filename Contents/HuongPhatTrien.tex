...
\section{Kết luận và Kết quả đạt được}

Đề tài đã hoàn thành các mục tiêu cốt lõi được đề ra, tập trung vào việc nghiên cứu và triển khai thành công hệ thống gợi ý đa phương thức tuần tự. Các kết quả nổi bật mà đồ án đã đạt được bao gồm:

\begin{itemize}
    \item \textbf{Về mặt lý thuyết:} Đồ án đã làm chủ và vận dụng thành công các kiến trúc học sâu tiên tiến, tiêu biểu là kiến trúc tháp đôi \textit{Bi-Encoder} cho pha truy xuất ứng viên và mô hình xếp hạng chéo \textit{Cross-Encoder} cho pha đánh giá chuyên sâu. Bên cạnh đó, hệ thống đã khai thác hiệu quả các mô hình biến đổi hiện đại như \textit{BERT} và \textit{Vision Transformer} để trích xuất đặc trưng đa phương thức một cách toàn diện.
    \item \textbf{Về mặt thực nghiệm:} Đồ án đã huấn luyện và đánh giá thành công mô hình trên các bộ dữ liệu thực tế quy mô lớn là \textit{Amazon Gift Cards} và \textit{All Beauty}. Hệ thống đã hoàn thiện luồng đánh giá toàn trình \textit{End-to-End} với các thước đo chuẩn xác như \textit{Hit Rate} và \textit{Recall}. Đặc biệt, quá trình nghiên cứu đã ứng dụng sáng tạo mô hình ngôn ngữ lớn để sinh tự động các truy vấn tổng hợp dạng dài, qua đó cải thiện đáng kể hiệu suất và độ chính xác của tầng xếp hạng \textit{Cross-Encoder}.
    \item \textbf{Về mặt kỹ thuật:} Mã nguồn được xây dựng ổn định bằng thư viện \textit{PyTorch}, kết hợp trơn tru với các nền tảng lưu trữ cơ sở dữ liệu vector. Toàn bộ luồng xử lý từ làm sạch dữ liệu, xây dựng tháp người dùng và tháp sản phẩm cho đến suy luận cuối cùng đều được chuẩn hóa thành một đường ống thống nhất.
\end{itemize}

\section{Hạn chế của đề tài}

Mặc dù đã đạt được những kết quả khả quan, kiến trúc hiện tại và quá trình triển khai thực nghiệm của đồ án vẫn còn tồn tại một số giới hạn nhất định:

\begin{itemize}
    \item \textbf{Giới hạn về tài nguyên điện toán:} Việc phụ thuộc vào các nền tảng huấn luyện đám mây có giới hạn ngặt nghèo về hạn mức sử dụng thẻ đồ họa máy tính \textit{GPU} đã tạo rào cản lớn. Điều này cản trở việc mở rộng kích thước lô dữ liệu \textit{batch size} cũng như thử nghiệm các cấu hình siêu tham số phức tạp hơn, khiến thời gian huấn luyện bị kéo dài và chưa khai thác tối đa tiềm năng của mạng nơ-ron.
    \item \textbf{Nút thắt cổ chai ở tầng xếp hạng chéo:} Dù mô hình \textit{Cross-Encoder} mang lại độ chính xác cực kỳ cao về mặt khớp ngữ nghĩa, việc tính toán mức độ tương quan cho từng cặp người dùng và sản phẩm lại tiêu tốn khối lượng tài nguyên khổng lồ. Độ trễ của hệ thống sẽ tăng vọt nếu phải xử lý hàng nghìn ứng viên cùng lúc trong môi trường sản xuất thực tế.
    \item \textbf{Thiếu hụt tính linh hoạt thời gian thực:} Hệ thống hiện tại chủ yếu được huấn luyện trên dữ liệu tĩnh và cập nhật theo từng đợt cố định. Sự vắng mặt của các luồng dữ liệu ngữ cảnh như thời gian mua sắm, thiết bị sử dụng hay chuỗi hành vi tương tác tức thời khiến mô hình gặp khó khăn trong việc nắm bắt các xu hướng thay đổi chóng vánh của người dùng.
    \item \textbf{Giới hạn trong phương pháp đánh giá:} Các thử nghiệm và số liệu đo lường hiện tại hoàn toàn được thực hiện trên dữ liệu lịch sử ngoại tuyến. Đề tài chưa có điều kiện triển khai hệ thống vào một ứng dụng thực tiễn để tiến hành đối chứng song song nhằm đo lường các chỉ số tương tác và tỷ lệ chuyển đổi sinh lời thật sự từ người dùng.
\end{itemize}

\section{Hướng phát triển hệ thống}

Trong môi trường công nghiệp, việc đánh giá hệ thống gợi ý không chỉ dừng lại ở các chỉ số bề mặt như Hit Rate mà phải hướng tới tối ưu hóa toàn bộ phễu chuyển đổi dựa trên mức độ tương tác thực tế của người dùng. Do đó, hướng phát triển cốt lõi đầu tiên là triển khai hệ thống thử nghiệm A/B (A/B Testing) trực tiếp. Tại đây, nhóm thử nghiệm sử dụng mô hình mới sẽ được so sánh song song với nhóm đối chứng dùng hệ thống cũ. Việc triển khai diện rộng chỉ được thực hiện khi mô hình mới chứng minh được sự tăng trưởng vững chắc của các chỉ số tương tác cốt lõi (Engagement Metrics), tương tự như mức tăng 0.37\% mà thuật toán hiệu chỉnh của YouTube đã đạt được trong thực tế~\cite{9}. Đối với một sàn thương mại điện tử, các thước đo định lượng này sẽ tập trung vào hành vi mua sắm sinh ra lợi nhuận, tiêu biểu là tỷ lệ chuyển đổi đơn hàng (Conversion Rate), tổng giá trị hàng hóa (GMV), tỷ lệ bấm thêm vào giỏ hàng (Add to Cart Rate) và thời gian khách hàng nán lại đọc bình luận (Dwell Time).

Bên cạnh việc đánh giá hệ thống, phương thức cập nhật mô hình cũng cần được tối ưu hóa. Trong kiến trúc hiện tại, mô hình Two-Tower phục vụ giai đoạn retrieval đang được vận hành và cập nhật theo cơ chế batch. Phương pháp này đòi hỏi phải huấn luyện lại toàn bộ dữ liệu lịch sử mỗi khi có luồng tương tác mới, gây lãng phí lớn về thời gian và tài nguyên tính toán. Ngược lại, nếu chỉ áp dụng fine-tuning trên các lô dữ liệu mới nhất, hệ thống sẽ đối mặt với rủi ro overfitting và hiện tượng catastrophic forgetting (quên đi các sở thích dài hạn của người dùng). 

Để giải quyết triệt để vấn đề này, hướng phát triển công nghệ tiếp theo là tích hợp phương pháp Sequential Meta-Learning~\cite{44}. Thay vì loại bỏ hoàn toàn mô hình cũ, phương pháp này huấn luyện một thành phần chuyển giao tri thức nhằm biến đổi các trọng số hiện tại để thích nghi nhanh với dữ liệu mới. Thành phần này được tối ưu trực tiếp theo hiệu năng trong tương lai, giúp hệ thống nhạy bén hơn với sự thay đổi hành vi theo thời gian thực của người dùng. Khi áp dụng vào mô hình Bi-Encoder hiện tại, giải pháp Meta-Learning giúp giảm thiểu tối đa chi phí huấn luyện lại, đồng thời cho phép cập nhật liên tục các item và user embedding một cách hiệu quả mà không cần xây dựng lại toàn bộ không gian vector.

\section{Thu hoạch của sinh viên}
...
Về mặt kỹ thuật chuyên môn, sinh viên đã từng bước vượt qua giới hạn của các thuật toán học máy cơ bản để làm chủ các kiến trúc học sâu phức tạp. Việc tự tay thiết kế, lập trình và tinh chỉnh các mạng \textit{Bi-Encoder} và \textit{Cross-Encoder} bằng thư viện \textit{PyTorch} giúp sinh viên hiểu sâu sắc về luồng di chuyển của dòng dữ liệu đa chiều (\textit{tensor}) và cơ chế tối ưu hóa trọng số bề sâu. Đặc biệt, quá trình làm việc với các tập dữ liệu thực tế quy mô lớn đã rèn luyện kỹ năng xử lý dữ liệu linh hoạt và năng lực quản lý tài nguyên phần cứng. Việc phải đối mặt với các giới hạn về bộ nhớ \textit{VRAM} và cách tối ưu hóa hạn mức sử dụng \textit{GPU} trên các nền tảng điện toán đám mây đòi hỏi sinh viên phải liên tục tối ưu hóa mã nguồn, tìm hiểu sâu về kỹ thuật cấp phát bộ nhớ để đảm bảo hệ thống có thể hội tụ mà không vượt quá ngưỡng phần cứng cho phép.

Về phương pháp nghiên cứu khoa học, đồ án đã rèn luyện thói quen tiếp cận tri thức trực tiếp từ các nguồn tài liệu gốc. Thay vì phụ thuộc vào các tài liệu hướng dẫn sẵn có, sinh viên đã hình thành kỹ năng phân tích, phản biện và tái hiện lại kiến trúc thuật toán từ các bài báo khoa học thuộc các hội nghị đầu ngành. Đồng thời, sự chuyển dịch tư duy từ việc chỉ quan tâm đến các độ đo đánh giá thuần túy sang việc nhìn nhận hệ thống dưới góc độ trải nghiệm người dùng cuối là một bước trưởng thành lớn trong nhận thức phát triển sản phẩm.

Cuối cùng, việc liên tục đối mặt với những thách thức trong quá trình huấn luyện, từ những lần thuật toán suy giảm hiệu suất đến các lỗi tràn bộ nhớ, đã mài giũa kỹ năng debugging hệ thống sâu, tính kiên định và khả năng quản lý thời gian độc lập. Những trải nghiệm này không chỉ củng cố nền tảng học thuật mà còn rèn luyện bản lĩnh thực chiến, giúp sinh viên Đại học Bách khoa tự tin đáp ứng những tiêu chuẩn khắt khe của môi trường công nghiệp thực tế.
