...
\subsection{Vai trò của LLM trong kiến trúc đường ống}
...
Để LLM hoạt động ổn định như một API backend thay vì một chatbot hội thoại thông thường, kỹ thuật thiết kế Prompt được xây dựng chặt chẽ theo hướng tiếp cận \textbf{RAG} kết hợp với \textbf{Đầu ra có cấu trúc}.

Cấu trúc Prompt được phân tách logic nhằm tối ưu hóa khả năng tuân thủ của mô hình:
\begin{itemize}
    \item \textbf{System Prompt:} Thiết lập vai trò (Trợ lý mua sắm AI), định nghĩa nhiệm vụ (phân tích lịch sử, chọn sản phẩm tốt nhất) và thiết lập các ràng buộc nghiêm ngặt (không bịa đặt thông tin, giới hạn số lượng kết quả).
    \item \textbf{User Prompt:} Dữ liệu thực tế được tiêm vào từ các giai đoạn trước, bao gồm truy vấn hiện tại, lịch sử tương tác và metadata chi tiết của Top-K sản phẩm ứng viên.
\end{itemize}
